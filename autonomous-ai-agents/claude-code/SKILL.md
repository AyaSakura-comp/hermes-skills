...
Connect your running Claude Code session to Discord or Telegram to message your agent from anywhere. Requires [Bun](https://bun.sh/) to be installed.

### Setup & Verification
1. **Install Bun:** `curl -fsSL https://bun.sh/install | bash`
2. **Install Discord Plugin:** `claude plugin install discord@claude-plugins-official`
...
### Key Considerations
- **Persistent Session:** The `claude` process must stay running (use tmux) for the channel to remain active.
- **Safety:** Channels carry prompt injection risks as inbound messages are pushed directly into the session.
- **Pairing:** You may need to pair your account in the chat platform on first use.

## PTY Dialog Handling (CRITICAL for Interactive Mode)

Claude Code presents up to two confirmation dialogs on launch. You MUST handle these via tmux send-keys:
...
❯ 1. Yes, I trust this folder    ← DEFAULT
  2. No, exit
```
**Handling:** `tmux send-keys -t <session> 1 Enter` or just `Enter` (since 1 is default).
...
### Robust Startup Script (via tmux)
```bash
# Start tmux
tmux new-session -d -s claude-session

# Launch with Discord channel and YOLO mode
tmux send-keys -t claude-session "claude --channels plugin:discord@claude-plugins-official --dangerously-skip-permissions" Enter

# Wait for and handle Trust prompt
sleep 5
tmux send-keys -t claude-session 1 Enter

# Wait for and handle Permissions bypass prompt (Down then Enter)
sleep 3
